...
# Создаем экземпляр для OneHotEncoder
...
def predict():
    ...
    input_data = pd.DataFrame([data], columns=['Object_area', 'Название процесса'])
    ...
    result = {
        "predicted_hours": int(predicted[0]),
        "predicted_price": int(predicted[1])
    }
